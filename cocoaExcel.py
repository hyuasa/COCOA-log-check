...
def create_cocoa_excel(logger, merge_df):
    ...
    # Excel保管
    bookname = 'COCOA_LOG_CHECKER_'+datetime.now(cc.JST).strftime('%Y-%m-%d-%H%M')+'.xlsx'
    ...
    wb = add_title_comment(logger, wb, cc.COCOA_EXPOSURE_SHEET_NAME, 3, 'cocoa_score', comment)
    wb = add_title_comment(logger, wb, cc.COCOA_EXPOSURE_SHEET_NAME, 3, '算出スコア計', comment)
    wb = add_chart(logger, wb, 'cocoa_score', ctitle='COCOA Score', y_title='スコア')
    wb = add_chart(logger, wb, '算出スコア計', ctitle='COCOA Calculated Score', y_title='スコア')
